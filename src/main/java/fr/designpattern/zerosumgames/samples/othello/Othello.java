...
public class Othello extends AbstractGame {
	...
	public boolean canPlayHere(final CellInterface cell, final OpponentsEnumeration side) {
		...
		if (cell.isEmpty()) {
			...
			for ( final Entry<BoardCardinalPosition, CellInterface> cellNeighbourEntry : cell.getNeighbourhood().entrySet()) {
	...
	public double computeStaticEvaluation(final LegalMoveInterface move) {
		...
		int n = 1;
		for ( final Entry<BoardCardinalPosition, CellInterface> cellNeighbourEntry : this.cell(move.getPosition()).getNeighbourhood().entrySet()) {
