...
public class Positions implements PositionsInterface {
    ...
    private final void setBoardPositionsCache(
            final PositionInterface[][] positions) {
    ...
    // ---------------------------------------------------------------------
    private final transient PositionInterface nullPosition = new GameBoardNullPosition(
            0, 0, -1, -1);
    ...
    // ---------------------------------------------------------------------
    private final PositionInterface createPosition(final int clientRowIndex,
            final int clientColumnIndex) {
        this.incrementNumberOfPositions();
        return new GameBoardPosition(clientRowIndex, clientColumnIndex,
                this.computeInternalRowIndex(clientRowIndex),
                this.computeInternalRowIndex(clientColumnIndex));
    }

    private final PositionInterface createNullPosition(
            final int clientRowIndex, final int clientColumnIndex) {
        return new GameBoardNullPosition(clientRowIndex, clientColumnIndex,
                this.computeInternalRowIndex(clientRowIndex),
                this.computeInternalRowIndex(clientColumnIndex));
    }
    ...
    private PositionInterface[][] createBoardPositionsCache() {
        return new PositionInterface[this.getBoardDimension()
                .getRowIndexRangeSize()][this.getBoardDimension()
                .getColumnIndexRangeSize()];
    }

    // ---------------------------------------------------------------------
    private PositionInterface[][] initializeBoardPositionsCache(
            final PositionInterface[][] positions) {
        ...
        for (int clientRowIndex = minRowIndex; clientRowIndex <= maxRowIndex; ++clientRowIndex) {
            for (int clientColumnIndex = minColumnIndex; clientColumnIndex <= maxColumnIndex; ++clientColumnIndex) {
                ...
                // "d : ", ++n);
                position = this.createPosition(clientRowIndex,
                        clientColumnIndex);
                // /System.out.print(position);
                positions[position.getInternalRowIndex()][position
                        .getInternalColumnIndex()] = position;
                // /System.out.print("\n");
            }
        }
    ...
    private PositionInterface[][] fillWithNullPositions(
            final PositionInterface[][] positions) {
        ...
        for (int clientRowIndex = minRowIndex; clientRowIndex <= maxRowIndex; ++clientRowIndex) {
            for (int clientColumnIndex = minColumnIndex; clientColumnIndex <= maxColumnIndex; ++clientColumnIndex) {
                nullPosition = this.createNullPosition(clientRowIndex,
                        clientColumnIndex);
                positions[nullPosition.getInternalRowIndex()][nullPosition
                        .getInternalColumnIndex()] = nullPosition;
                // System.out.print("\n");
            }
        }
    ...
    public Positions(final DimensionInterface boardDimension) {
        ...
        this.setBoardPositionsCache(this.initializeBoardPositionsCache(this
                .fillWithNullPositions(this.createBoardPositionsCache())));
    }

    // ---------------------------------------------------------------------
    public PositionInterface position(final int clientRowIndex,
            final int clientColumnIndex) {

        PositionInterface position;

        // si la position est hors-dimension, nulle position n'est retournée.
        if (this.getBoardDimension()
                .contains(clientRowIndex, clientColumnIndex)) {
            position = this.getBoardPositions()[this
                    .computeInternalRowIndex(clientRowIndex)][this
                    .computeInternalColumnIndex(clientColumnIndex)];
        }
        else {
        ...
        if (position == null) {
            throw new GameBoardIllegalPositionException(clientRowIndex,
                    clientColumnIndex);
        }
    ...
    private PositionInterface getAdjacentPositionTo(
            final PositionInterface position,
            final BoardCardinalPosition direction) {
        return this.position(
                position.getClientRowIndex() + direction.getDeltaRowIndex(),
                position.getClientColumnIndex()
                        + direction.getDeltaColumnIndex());
    }
    ...
    public PositionInterface rightOf(final PositionInterface position) {
        return this
                .getAdjacentPositionTo(position, BoardCardinalPosition.RIGHT);
    }
    ...
    public PositionInterface bottomOf(final PositionInterface position) {
        return this.getAdjacentPositionTo(position,
    ...
    private class GameBoardIllegalPositionException extends RuntimeException {
        ...
        public GameBoardIllegalPositionException(final int clientRowIndex,
                final int clientColumnIndex) {
            super();
    ...
    private class GameBoardNullPosition implements PositionInterface {
        ...
        // ---------------------------------------------------------------------
        public GameBoardNullPosition(final int clientRowIndex,
                final int clientColumnIndex, final int internalRowIndex,
                final int internalColumnIndex) {
    ...
    private class GameBoardPosition extends GameBoardNullPosition {
        ...
        // ---------------------------------------------------------------------
        public GameBoardPosition(final int clientRowIndex,
                final int clientColumnIndex, final int internalRowIndex,
                final int internalColumnIndex) {
            super(clientRowIndex, clientColumnIndex, internalRowIndex,
                    internalColumnIndex);
        }
        ...
        @Override
        public String toString() {
            return "[row " + this.getClientRowIndex() + "][column "
                    + this.getClientColumnIndex() + "]";
        }
        // ---------------------------------------------------------------------
    }
    // =====================================================================
